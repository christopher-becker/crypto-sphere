...
export async function getCoinsList (currency: string = "usd") {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch coins list data: ${response.status} ${response.statusText}`);
        }
...
export async function getCoinDetail (id: string) {
    try {
        ...
        const response = await fetch(endpoint);
        if (!response.ok) {
            throw new Error(`Failed to fetch coin detail: ${response.status} ${response.statusText}`);
        }
        return await response.json();
    } catch (error) {
        console.error(error);
